...
def vals_to_hex(vals):
    ...
    assert args.link_bitwidth >= tnb, "Attempting to pack {} x {} bits ({} bits) into {}".format(len(vals), nb, tnb, args.link_bitwidth)
...
parser.add_argument('--pack_links', type=bool, help='Whether to pack multiple values into one link where possible')
...
if fp32_data.shape[1] > link_width * pf:
    raise Exception('Trying to fit {} features into {} links with packing factor {}'.format(fp32_data.shape[1],link_width,pf))
...
link_map = list(range(args.link_range[0], args.link_range[1] + 1)) if args.link_map is None else args.link_map
assert len(link_map) == link_width, 'Link map length ({}) does not match link range ({})'.format(len(link_map), link_width)
